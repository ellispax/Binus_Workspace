...
def create_fleet(ai_settings, screen, ship, aliens):
	...
	# alien = Alien(ai_settings, screen)
	number_aliens_x = get_number_aliens_x(ai_settings, ship.rect.width)
...
def check_keydown_events(event, ai_settings, screen, ship, bullets):
	...
	# Spacebar - shooting
		...
		fire_bullet(ai_settings, screen, ship, bullets, False)
	elif event.key == pygame.K_q:
		sys.exit()

			
def fire_bullet(ai_settings, screen, ship, bullets, isEnemy):
	"""Fire a bullet if limit is not reached yet."""
	# Create a new bullet and add it to the bullets group
	if len(bullets) < ai_settings.bullets_allowed:
		new_bullet = Bullet(ai_settings, screen, ship, isEnemy)
...
def update_bullets(ai_settings, screen, ship, aliens, bullets):
	...
	for bullet in bullets.copy():
		if bullet.isEnemy and bullet.rect.top >= ai_settings.screen_height:
	...
	for bullet in bullets.sprites():
		if bullet.isEnemy:
			if bullet.rect.colliderect(ship.rect):
				if time.time() - ship.time >= ship.invincible_time:
					ship.health -= 1
					ship.time = time.time()
				if ship.health == 0:
					# lose
